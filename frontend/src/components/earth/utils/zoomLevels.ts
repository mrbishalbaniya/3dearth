...
export const ZOOM_LEVELS: ZoomLevelDef[] = [
  ...
  },
  {
    ...
    maxAltitudeM: 250,
    minAltitudeM: 10,
    description: "Ground detail · ~10 m eye height",
    tileZoom: 19,
    showStars: false,
    showAtmosphere: false,
    showClouds: false,
...
export function altitudeToZoomLevel(altitudeM: number): ZoomLevelDef {
  const clamped = Math.max(10, altitudeM);
...
export function altitudeMToOrbitDistance(altitudeM: number): number {
  return Math.max(altitudeM / EARTH_RADIUS_M, 10 / EARTH_RADIUS_M);
}
...
/**
 * Mercator tile Z from altitude.
 * Aligned with ZOOM_LEVELS: globe carries orbit/continent; tiles start sharp at country+.
 */
export function altitudeToTileZoom(altitudeM: number): number {
  // Explicit ladder — avoids soft Z5–6 blobs over the day map at ~2 Mm
  if (altitudeM > 4_000_000) return 2;
  if (altitudeM > 1_500_000) return 5;
  if (altitudeM > 800_000) return 7;
  if (altitudeM > 400_000) return 9;
  if (altitudeM > 200_000) return 10;
  if (altitudeM > 80_000) return 11;
  if (altitudeM > 40_000) return 12;
  if (altitudeM > 15_000) return 13;
  if (altitudeM > 5_000) return 14;
  if (altitudeM > 1_500) return 15;
  if (altitudeM > 500) return 16;
