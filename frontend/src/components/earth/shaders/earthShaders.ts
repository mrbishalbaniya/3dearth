 ...
 * night lights, AO from normals, coast blending + Dry Earth hypsometry.
 */
import { HYPSO_GLSL } from "../dryEarth/hypsometric";
...
void main() {
  ...
  // Dry Earth on the globe mesh (avoids second-shell z-fighting bands)
  float dryOcean = ocean;
  if (uDryBlend > 0.001) {
    float lum = dot(dayColor, vec3(0.299, 0.587, 0.114));
    float green = dayColor.g - dayColor.r * 0.5;
    // Land elevation from albedo + normals (mountains darker / rougher)
    float landElev = mix(-20.0, 5200.0, clamp(lum * 0.42 + (1.0 - green) * 0.38 + roughnessSample * 0.35, 0.0, 1.0));
    landElev += (noise(vUv * 48.0) - 0.5) * 700.0;
    landElev += polarMask * 2200.0;
    // Snow peaks + brown highlands so Himalaya / Tibet aren't flat green
    float snow = smoothstep(0.62, 0.88, min(min(dayColor.r, dayColor.g), dayColor.b));
    float highland = smoothstep(0.02, 0.28, dayColor.r - dayColor.g) *
      smoothstep(0.25, 0.55, lum) * (1.0 - snow);
    landElev = mix(landElev, mix(2800.0, 5200.0, lum), highland);
    landElev = mix(landElev, mix(5000.0, 8800.0, lum), snow);
    landElev = clamp(landElev, -20.0, 8850.0);
    // Ocean floor vs MSL (0): darker / more specular → deeper basins
    float depthT = clamp(
      (1.0 - lum) * 0.55 + (1.0 - dayColor.b) * 0.35 + specularMask * 0.45,
      0.0,
      1.0
    );
    float ridge = noise(vUv * 22.0 + vec2(2.2, 4.1));
    float trench = pow(noise(vUv * 9.0 + 5.5), 2.8);
    // Depth below mean sea level (negative meters)
    float oceanElev = mix(-120.0, -5800.0, depthT) + ridge * 900.0 - trench * 5200.0;
    oceanElev = clamp(oceanElev, -10950.0, -15.0);
    float elev = mix(oceanElev, landElev, land);
    float exposed = smoothstep(uSeaLevelM - 40.0, uSeaLevelM + 40.0, elev);
    vec3 hypo = hypsometricColor(elev);
    // Fully drained look: seafloor + mountains, no residual blue water / satellite brown
    vec3 dryLand = hypo;
    vec3 dryOceanCol = hypo;
    vec3 dryLook = mix(dryOceanCol, dryLand, land);
    float shore = 1.0 - smoothstep(0.0, 80.0, abs(elev - uSeaLevelM));
    dryLook = mix(dryLook, vec3(0.92, 0.88, 0.42), shore * 0.25 * exposed * (1.0 - land));
    // When sea is fully drained, force all ocean to seafloor colors
    float fullyDry = 1.0 - smoothstep(-2000.0, -200.0, uSeaLevelM);
    dryLook = mix(mix(dryOceanCol, dryLand, land), dryLook, 1.0 - fullyDry * ocean);
    dryLook = mix(dryLook, hypo, fullyDry * ocean);
    // Kill Blue Marble land underlay — it read as a fake "brown crust" in dry mode
    surfaceColor = mix(surfaceColor, dryLook, clamp(uDryBlend, 0.0, 1.0));
    surfaceColor = mix(surfaceColor, hypo, fullyDry * uDryBlend * land);
    // Kill water specular / fresnel — no oceans, rivers, lakes look
    dryOcean = ocean * (1.0 - uDryBlend) * (1.0 - exposed);

    // Punch ocean holes so displaced seafloor (below r=1) is visible
    if (uDryBlend > 0.75 && ocean > 0.55 && uSeaLevelM < -500.0) {
      discard;
    }
  }

  // Roughness / specular (GGX-ish Blinn)
  float roughness = mix(0.42, 0.045, dryOcean);
  ...
  vec3 specular =
    uSunColor *
    spec *
    mix(0.08 * land + land * uWetness * 0.55, 1.55 + fresnelSpec * 2.2 * (1.0 + uWaveStorm), dryOcean) *
    mix(1.0, 1.8, ice);
