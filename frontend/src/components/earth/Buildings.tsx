...
async function fetchOsmBuildings(
  ...
  try {
    const { bboxAround, fetchBuildingsDetailed } = await import("./gis/overpass");
    const rows = await fetchBuildingsDetailed(
      bboxAround(lat, lng, 0.008),
      signal,
    );
    return rows.slice(0, maxCount).map((e, i) => ({
      id: e.id,
      lat: e.lat,
      lng: e.lng,
      height: Math.min(e.height, 280),
      width: 14 + (i % 5) * 4,
      depth: 12 + (i % 7) * 3,
      color: e.building === "commercial" ? "#6a7a8c" : "#8a9098",
      name: e.name,
    }));
  } catch {
    return null;
...
export function Buildings({ quality }: BuildingsProps) {
  ...
  useEffect(() => {
    ...
    fetchOsmBuildings(latS, lngS, quality.maxBuildings, ctrl.signal).then(
      (result) => {
