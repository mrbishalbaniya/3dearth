...
export function TerrainLayer() {
  ...
  // Dry Earth uses BathymetryLayer (full ocean depth) — skip this land-only DEM
  // Keep DEM near the surface in flight so highland runways aren't empty blue
  const show =
    !dryEarthOn &&
    (gis.terrain || gis.elevation) &&
    altitudeM < 180_000 &&
    (altitudeM > 250 || flightMode);

  const altQ = useMemo(() => {
    if (altitudeM > 50_000) return Math.round(altitudeM / 10_000) * 10_000;
    if (altitudeM > 10_000) return Math.round(altitudeM / 2_000) * 2_000;
    if (altitudeM > 2_000) return Math.round(altitudeM / 400) * 400;
    return Math.round(altitudeM / 80) * 80;
  }, [altitudeM]);
  ...
  const { tiles, generation, gridSize } = useMemo(() => {
    if (!show) return { tiles: [], generation: generationRef.current, gridSize: 33 };
    ...
    const center = sel.dem.find((t) => t.role === "detail") ?? sel.dem[0];
    const genKey = center
      ? `${sel.targetZ}/${center.x}/${center.y}`
      : `${sel.targetZ}`;
    if (lastKeyRef.current !== genKey) {
      lastKeyRef.current = genKey;
  ...
  }, [show, focusLat, focusLng, altQ, quality, zoomLevel]);
