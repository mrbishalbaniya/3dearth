/**
 * Elevation sampling — land (Terrarium) + ocean depth below MSL (Seascape/GEBCO).
 * Uses shared DemGridCache to avoid duplicate decode with Terrain/Bathymetry layers.
 */
import {
  buildTileUrl,
  BATHYMETRY_DEM_URL,
  TERRARIUM_DEM_URL,
  tileToLngLatBounds,
} from "../gis/TileLoader";
import {
  demCacheKey,
  loadDemGrid,
  peekDemGrid,
  storeDemGrid,
} from "../cache/DemGridCache";
import { lngLatToTile } from "../utils/tiles";

interface ElevTile {
  z: number;
  x: number;
  y: number;
  size: number;
  data: Float32Array;
  lastUsed: number;
}

const cache = new Map<string, ElevTile>();
const inflight = new Map<string, Promise<ElevTile>>();
/** Interim MSL heights (e.g. airport elev) until real DEM tiles land. */
const elevProxies = new Map<string, number>();
const MAX = 64;

function mergeLandOcean(land: Float32Array, ocean: Float32Array): Float32Array {
  const out = new Float32Array(land.length);
  for (let i = 0; i < out.length; i++) {
    const L = land[i];
    const O = ocean[i];
    if (L > 2) out[i] = L;
    else if (O < -1) out[i] = O;
    else if (L < -1) out[i] = L;
    else out[i] = Math.min(L, O);
  }
  return out;
}

async function loadElevTile(
  z: number,
  x: number,
  y: number,
  size = 64,
): Promise<ElevTile> {
  const key = `${z}/${x}/${y}/${size}`;
  const hit = cache.get(key);
  if (hit) {
    hit.lastUsed = performance.now();
    return hit;
  }
  const pending = inflight.get(key);
  if (pending) return pending;

  const promise = (async () => {
    const mergedKey = demCacheKey("merged", z, x, y, size);
    let heights = peekDemGrid(mergedKey);
    if (!heights) {
      const landUrl = buildTileUrl(TERRARIUM_DEM_URL, z, x, y);
      const oceanUrl = buildTileUrl(BATHYMETRY_DEM_URL, z, x, y);
      const landKey = demCacheKey("land", z, x, y, size);
      const oceanKey = demCacheKey("ocean", z, x, y, size);
      const [land, ocean] = await Promise.all([
        loadDemGrid(landKey, landUrl, size).catch(() => null),
        loadDemGrid(oceanKey, oceanUrl, size).catch(() => null),
      ]);
      if (!land && !ocean) throw new Error("elevation tile failed");
      heights =
        land && ocean ? mergeLandOcean(land, ocean) : (land ?? ocean)!;
      storeDemGrid(mergedKey, heights, size);
    }
    const tile: ElevTile = {
      z,
      x,
      y,
      size,
      data: heights,
      lastUsed: performance.now(),
    };
    cache.set(key, tile);
    if (cache.size > MAX) {
      let oldestKey: string | null = null;
      let oldest = Infinity;
      for (const [k, v] of cache) {
        if (v.lastUsed < oldest) {
          oldest = v.lastUsed;
          oldestKey = k;
        }
      }
      if (oldestKey) cache.delete(oldestKey);
    }
    return tile;
  })();

  inflight.set(key, promise);
  try {
    return await promise;
  } finally {
    inflight.delete(key);
  }
}

export async function sampleElevation(
  lat: number,
  lng: number,
  z = 10,
): Promise<number> {
  const { x, y } = lngLatToTile(lng, lat, z);
  const tile = await loadElevTile(z, x, y, 64);
  const bounds = tileToLngLatBounds(z, x, y);
  const u = (lng - bounds.lngMin) / (bounds.lngMax - bounds.lngMin);
  const v = (bounds.latMax - lat) / (bounds.latMax - bounds.latMin);
  const fx = Math.min(tile.size - 1.001, Math.max(0, u * (tile.size - 1)));
  const fy = Math.min(tile.size - 1.001, Math.max(0, v * (tile.size - 1)));
  const x0 = Math.floor(fx);
  const y0 = Math.floor(fy);
  const x1 = Math.min(tile.size - 1, x0 + 1);
  const y1 = Math.min(tile.size - 1, y0 + 1);
  const tx = fx - x0;
  const ty = fy - y0;
  const h00 = tile.data[y0 * tile.size + x0];
  const h10 = tile.data[y0 * tile.size + x1];
  const h01 = tile.data[y1 * tile.size + x0];
  const h11 = tile.data[y1 * tile.size + x1];
  const h0 = h00 * (1 - tx) + h10 * tx;
  const h1 = h01 * (1 - tx) + h11 * tx;
  return h0 * (1 - ty) + h1 * ty;
}

export function peekElevation(lat: number, lng: number, z = 10): number | null {
  const { x, y } = lngLatToTile(lng, lat, z);
  const key = `${z}/${x}/${y}/64`;
  const tile = cache.get(key);
  if (!tile) {
    const proxy = elevProxies.get(`${z}/${x}/${y}`);
    return proxy != null ? proxy : null;
  }
  const bounds = tileToLngLatBounds(z, x, y);
  const u = (lng - bounds.lngMin) / (bounds.lngMax - bounds.lngMin);
  const v = (bounds.latMax - lat) / (bounds.latMax - bounds.latMin);
  const ix = Math.min(
    tile.size - 1,
    Math.max(0, Math.round(u * (tile.size - 1))),
  );
  const iy = Math.min(
    tile.size - 1,
    Math.max(0, Math.round(v * (tile.size - 1))),
  );
  return tile.data[iy * tile.size + ix];
}

/**
 * Seed a constant MSL height for a tile cell so flight/imagery can frame
 * highland airports before Terrarium DEM arrives. Real tiles replace this.
 */
export function seedElevationProxy(
  lat: number,
  lng: number,
  elevM: number,
  z = 10,
) {
  const { x, y } = lngLatToTile(lng, lat, z);
  elevProxies.set(`${z}/${x}/${y}`, elevM);
}

export function warmElevation(lat: number, lng: number, z = 10) {
  const { x, y } = lngLatToTile(lng, lat, z);
  void loadElevTile(z, x, y, 64).catch(() => undefined);
}
