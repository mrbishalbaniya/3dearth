...
function detectGpuTier(): EarthQualityProfile["id"] {
  ...
  // Never auto-pick ultra — 8K earth maps + Z18 tiles crash Chrome (STATUS_BREAKPOINT).
  // discrete GPUs → high max; iGPU / unknown → medium; weak → low.
  if (
...
export function useGpuTier(): EarthQualityProfile {
  ...
  const [profile, setProfile] = useState<EarthQualityProfile>(
    () => QUALITY_PROFILES[useEarthStore.getState().qualityId] ?? QUALITY_PROFILES.medium,
  );
