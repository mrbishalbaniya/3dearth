...
export function LayerSidebar() {
  ...
  const [openSection, setOpenSection] = useState<SectionId | null>("dryearth");
  ...
  return (
    <div
      className={`earth-sidebar-root ${sidebarOpen ? "earth-sidebar-root--open" : ""}`}
    >
      <button
        type="button"
        className={`earth-sidebar-fab ${sidebarOpen ? "earth-sidebar-fab--open" : ""}`}
        onClick={() => setSidebarOpen((v) => !v)}
        aria-expanded={sidebarOpen}
        aria-label={sidebarOpen ? "Close layers sidebar" : "Open layers sidebar"}
        title="Layers"
      >
        {sidebarOpen ? "✕" : "☰"}
      </button>
      ...
      {sidebarOpen && (
        <aside className="earth-sidebar" aria-label="Layers and settings">
          <div className="earth-sidebar__header">
            <div>
              <div className="earth-sidebar__title">Layers</div>
              <div className="earth-sidebar__sub">Toggle map & scene</div>
            </div>
          </div>
              ...
              {GIS_GROUPS.map((group) => {
                ...
                return (
                  <div key={group.id} className="earth-sidebar__group">
                    <div className="earth-sidebar__group-label">{group.label}</div>
                    {items.map((meta) => (
                      <Switch
                        key={meta.id}
