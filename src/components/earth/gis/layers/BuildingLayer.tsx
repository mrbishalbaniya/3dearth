...
import { vectorScheduler } from "../../streaming";
...
export function BuildingLayer() {
  ...
  useEffect(() => {
    if (!enabled || zoomLevel < 5 || altitudeM > 30_000) {
      setBuildings([]);
      return;
    }
  ...
  }, [enabled, q.lat, q.lng, altitudeM, zoomLevel, max]);
  ...
  const instances = useMemo(() => {
    ...
    return buildings.map((b) => {
      const base = latLngToVector3(b.lat, b.lng, EARTH_RADIUS * 1.0016);
  ...
  if (!enabled || !instances.length) return null;
